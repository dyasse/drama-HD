  ...
  return (
    <main className="space-y-5">
      ...
      <div className="flex flex-wrap gap-2">
        {curatedMovieFilters.map((genre) => (
          <Link key={genre.label} href={`/${locale}/genres/${genre.id}?type=movie`} className="rounded-full bg-emerald px-3 py-1 text-sm text-cream">
            {genre.label}
          </Link>
        ))}
      </div>
